 ...
 *
 * Props:
 * - activetasks / statictasks (both from app)
 * - onReload (callback which is called when an active task changes to complete, not for static tasks)
 */
export default class Tasks extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if(prevState.active_tasks && prevState.active_tasks.length > 0) {
            Object.entries(prevState.active_tasks).forEach(([k, v]) => {
                if(this.state.active_tasks
                    && this.state.active_tasks[k]
                    && this.state.active_tasks[k].progress == 100
                    && v.progress < 100) {
                    // Entry changed to 100% now -> trigger reload
                    this.props.onReload()
                }
            })
        }
